...
class Actor(nn.Module):
    ...
    def forward(self, x):
        # print("\nWhat is X :", x.double(), type(x), x.shape) #shape is of state dim only not both
        hidden = self.net(x)
    ...
    def train(self, load):
        ...
        for eps in tqdm(range(TRAIN_STEPS), desc='episodes'): 
            ...
            for timestep in range(max_timesteps):
                ...
                #print("The state ", state, type(state), state.shape)
                
                # Returns [X, Y] where X + Y = 1. Which action to take, but we need value of action as well
